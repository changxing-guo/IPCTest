...
public class MessengerTestActivity extends Activity {
    ...
    /**
     * 总结
     * 客户端就主要是发起与服务端的绑定，以及通过onServiceConnected()方法来过去服务端返回来的IBinder，借此构造Messenger，
     * 从而可以通过发送Message的方式与服务端进行交互。上面的例子其实并不完整，因为它只有客户端对服务端单方面的通信，而服务端
     * 没有发信息给客户端的功能——这显然是不合理的。而要实现这个其实也很简单，只要客户端里也创建一个Handler实例，让它接收来自
     * 服务端的信息，同时让服务端在客户端给它发的请求完成了之后再给客户端发送一条信息即可。
     *
